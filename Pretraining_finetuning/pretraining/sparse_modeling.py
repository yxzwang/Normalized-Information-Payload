...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        sparsity_config = TesseractSparsityConfig(config.num_attention_heads, 16)
        self.self = deepspeed_sparse_attn.BertSparseSelfAttention(config, sparsity_config)

        self.output = BertSelfOutput(config)

    def forward(self, input_tensor, attention_mask):
        # key_padding_mask_mode is "add" by default in deepspeed.ops.sparse_attention.BertSparseSelfAttention
        key_padding_mask = attention_mask.squeeze(1).squeeze(1) # (bsz, seq_len)
        # key_padding_mask[key_padding_mask == -10000.] = -float('inf')
        # key_padding_mask = key_padding_mask.half()

        dtype = input_tensor.dtype
        input_tensor = input_tensor.half()
        context_layer = self.self(input_tensor, key_padding_mask)
