...
class CosineCurve:
    ...
    def get_decay(self, total, current, total_warmup):
        num_cycles = 0.5
        progress = float(current - total_warmup) / float(max(1, total - total_warmup))
...
CURVES = {
    "linear": lambda args: LinearCurve(), 
    "exp": lambda args: ExpCurve(args),
    "cosine": lambda args: CosineCurve(),
}
