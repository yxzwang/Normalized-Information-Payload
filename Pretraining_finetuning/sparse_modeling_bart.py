...
class BartAttention(nn.Module):
    ...
    def __init__(
        ...
        import deepspeed.ops.sparse_attention as deepspeed_sparse_attn
        from deepspeed.ops.sparse_attention import SparseSelfAttention,BigBirdSparsityConfig
        bigbird_config=BigBirdSparsityConfig(num_heads, block=16, num_global_blocks=1, num_random_blocks=4, num_sliding_window_blocks=3)
        longformer_config=BigBirdSparsityConfig(num_heads, block=16, num_global_blocks=1, num_random_blocks=0, num_sliding_window_blocks=3)
        sparsity_config = deepspeed_sparse_attn.TesseractSparsityConfig(num_heads, 16)
    ...
    def forward(
        ...
        context_layer = self.sparse_self_attention(
            query_states, key_states, value_states, key_padding_mask=attention_mask
        )
        # attn_weights = torch.bmm(query_states, key_states.transpose(1, 2))
        ...
        # attn_output = torch.bmm(attn_probs, value_states)
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()  ##blhe
...
class BartEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        return outputs
class BartAttentionD(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
class BartDecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = BartAttentionD(
            self.embed_dim,
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    # def resize_position_embeddings(self, max_seq: int) -> nn.Embedding:
    #     max_pos=4096
