...
from pretraining.sparse_base import BasePretrainModel
...
def create_finetune_job(args, index, global_step, model):
    try:
        ...
        # no training afterward. safe to drop master_layout
        # need this to read model weights correctly in run_glue_sparse.py
        for layer in model.network.module.base_model.encoder.layer:
            layer.attention.self.sparse_self_attention.master_layout = None
...
def prepare_model_and_optimizer(args):
    ...
    # from pretraining.sparse_modeling import BertLMHeadModel
    # model.network = BertLMHeadModel.from_pretrained("tbert240k/epoch1000000_step240018")

    # Optimizer parameters
    # no weight decay for ["bias", "LayerNorm.bias", "LayerNorm.weight"]
