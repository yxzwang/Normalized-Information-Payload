...
task = task if not task.endswith("-cls") else task[:-4]
ds_iter = {
...
optimizer = torch.optim.AdamW(
    ...
    betas = (0.9, 0.98),
    eps = 1e-6, weight_decay = training_config["weight_decay"]
)
...
def step(component, step_idx):
    ...
    print(f"model={args.model}, task={args.task}, step={step_idx}, tt={time_since_start:.1f}, t={t_escape:.3f}, bs={batch_size}, lr={learning_rate:.6f}, loss={loss:.4f}, accu={accu:.4f}\t\t\t\t", end = "\r", flush = True)
...
def print_summary(summary, save_if_improved, train_step_idx):
    ...
    if summary["accu"] > summary["best_accu"]:
        ...
        if save_if_improved:
            ...
            print(f"model={args.model}, task={args.task}, best_accu={best_accu}. Saved best model")
